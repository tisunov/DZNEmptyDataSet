...
- (void)updateConstraints
{
    ...
    if (!self.didCenterToSuperview) {
        ...
        if (!CGPointEqualToPoint(self.offset, CGPointZero) && self.constraints.count == 4) {
            ...
            vConstraint.constant = self.offset.y*-2;
        }
    }
    ...
    if (_button.superview) {
        ...
        [_contentView addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-40-[button]-40-|"
                                                                             options:0 metrics:metrics views:views]];
    }
...
- (void)dzn_configureButtonAppearance
{
    if (self.emptyDataSetSource && [self.emptyDataSetSource respondsToSelector:@selector(configureButtonAppearance:)]) {
        [self.emptyDataSetSource configureButtonAppearance:self.emptyDataSetView.button];
    }
}
    ...
    if ([self dzn_shouldDisplay] && [self dzn_itemsCount] == 0)
    {
        ...
        [UIView performWithoutAnimation:^{
            ...
            if (customView) {
                ...
                [self dzn_configureButtonAppearance];
